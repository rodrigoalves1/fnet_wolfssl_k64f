 ...
 */
fnet_return_t SMTP_ssl_send_email (fnet_shell_desc_t desc,
				SMTP_PARAM_STRUCT_PTR params, char *err_string,
						fnet_uint32_t err_string_size) {
	char *response = NULL;
	char *command = NULL;
	char *location = NULL;
	...
	if ((params == NULL) ||
	(params->envelope.from == NULL) || (params->envelope.to == NULL)) {
	...
	fnet_shell_println(desc, "Connection to gmail took %u.%u seconds",
