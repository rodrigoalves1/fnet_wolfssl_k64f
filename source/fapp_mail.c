 ...
 *
 * @brief New FNET shell functionalities Mail, TX/RX TLS Benchmark.
 *
 */
#ifndef BENCHMARK
#include "fapp.h"
...
{
	/* Listening socket.*/
	fnet_socket_t socket_listen;
	/* Foreign socket.*/
	fnet_socket_t socket_foreign;
	/* Transmit circular buffer */
	fnet_uint8_t buffer[FAPP_BENCH_PACKET_SIZE_MAX];
...
*     Function Prototypes
*************************************************************************/
static void fapp_dns_handler_resolved (const struct fnet_dns_resolved_addr
		*addr_list, fnet_size_t addr_list_size, fnet_uint32_t cookie);
...
void fapp_client( fnet_shell_desc_t desc, fnet_index_t argc, fnet_char_t **argv ) {
	 ...
	 * Make sure both client and server uses the same cert/key type
	 */
	...
	if(packet_size > (8*1024)) /* Check max size.*/
		packet_size = (8*1024);
	/*
	 * The test may use IPv6 or IPv4
	 * E.g. IPv6: fe80::98f4:1bd1:fdf8:c4f8%3
	 * E.g. IPv4: 192.168.3.100
	 */
	...
	/* Uncomment to see server supported ciphers*/
	// ShowCiphers();
	...
	while(iterations--) {
		...
		if (wolfSSL_connect(ssl) != SSL_SUCCESS)
			...
			fnet_shell_println(desc,
				"Connection to server took %u.%u seconds",
				((interval * FNET_TIMER_PERIOD_MS) / 1000),
				((interval * FNET_TIMER_PERIOD_MS) % 1000) / 100);
...
void fapp_server( fnet_shell_desc_t desc, fnet_index_t argc, fnet_char_t **argv ){
	...
	fnet_char_t	ip_str[FNET_IP_ADDR_STR_SIZE];
	struct linger	linger_option = {FNET_TRUE, /*l_onoff*/ 4  /*l_linger*/};
	...
	if (wolfSSL_CTX_set_cipher_list(ctx, "AES128-GCM-SHA256") != SSL_SUCCESS)
		fnet_shell_println(desc, "Error cipher.");
	...
	/*
	 * This certificate will be treated as trusted root certificates and
...
void fapp_mail( fnet_shell_desc_t desc, fnet_index_t argc, fnet_char_t **argv ){
	...
	params.envelope.from = "mail@gmail.com";
	params.envelope.to = "mail-to@domain.com";
	params.login = "mail@gmail.com";
	...
	if(fnet_inet_ptos(server, &params.server) == FNET_OK){
		...
		dns_params.host_name = server;
		/* Callback function.*/
		dns_params.handler = fapp_dns_handler_resolved;
		/* Application-specific parameter
		 * which will be passed to fapp_dns_handler_resolved().
		 */
		dns_params.cookie = (fnet_uint32_t)desc;
