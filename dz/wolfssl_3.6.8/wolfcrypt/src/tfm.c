...
void fp_mul(fp_int *A, fp_int *B, fp_int *C)
{
    ...
    if (ptrA && ptrB && ptrC)
    {
        ...
        LTC_PKHA_ModMul(LTC_BASE, ptrA, sizeA, ptrB, sizeB, ptrC, LTC_MAX_INT_BYTES, ptrB, &sizeB, kLTC_PKHA_IntegerArith, 
    ...
    /* fix sign */
    C->sign = neg;
    if (ptrA) {XFREE(ptrA, 0, DYNAMIC_TYPE_BIGINT);}
    if (ptrB) {XFREE(ptrB, 0, DYNAMIC_TYPE_BIGINT);}
    if (ptrC) {XFREE(ptrC, 0, DYNAMIC_TYPE_BIGINT);}
    return;
#else /* FREESCALE_LTC_TFM */
    int   y, yy;
...
//#if 0
    ...
    if (ptrA && ptrB && ptrC)
    {
        ...
        if (kStatus_Success == LTC_PKHA_ModRed(LTC_BASE, ptrA, sizeA, ptrB, sizeB, ptrC, &sizeC, kLTC_PKHA_IntegerArith))
        {
...
//#if 0
    ...
    if (ptrA && ptrB && ptrC)
    {
        ...
        if (kStatus_Success == LTC_PKHA_ModInv(LTC_BASE, ptrA, sizeA, ptrB, sizeB, ptrC, &sizeC, kLTC_PKHA_IntegerArith))
        {
            ltc_reverse_array(ptrC, sizeC);   
            mp_read_unsigned_bin(c, ptrC, sizeC);
        }
...
//#if 0
    ...
    /* if A or B is negative, substracta abs(A) or abs(B) from modulus to get positive integer representation of the same number */
    fp_init(&t);
    if (a->sign)
    {
        fp_add(a,c,&t);
    ...
    if (ptrA && ptrB && ptrC && ptrD)
    {
        ...
        {
            if (kStatus_Success != LTC_PKHA_ModRed(LTC_BASE, ptrA, sizeA, ptrC, sizeC, ptrA, &sizeA, kLTC_PKHA_IntegerArith))
            {
                res = FP_VAL;
            }
        }
        ...
        {
            if (kStatus_Success != LTC_PKHA_ModRed(LTC_BASE, ptrB, sizeB, ptrC, sizeC, ptrB, &sizeB, kLTC_PKHA_IntegerArith))
            {
                res = FP_VAL;
            }
        }
    ...
    if (ptrB) {XFREE(ptrB, 0, DYNAMIC_TYPE_BIGINT);}
    if (ptrC) {XFREE(ptrC, 0, DYNAMIC_TYPE_BIGINT);}
    if (ptrD) {XFREE(ptrD, 0, DYNAMIC_TYPE_BIGINT);}
    return res;
#else   /* FREESCALE_LTC_TFM */
  fp_int tmp;
  fp_init(&tmp);
...
//#if 0
    ...
    if (ptrG && ptrX && ptrP)
    {
        ...
        {
            res = (int)LTC_PKHA_ModRed(LTC_BASE, ptrG, sizeG, ptrP, sizeP, ptrG, &sizeG,
                                           kLTC_PKHA_IntegerArith);
    ...
    if (ptrG) {XFREE(ptrG, 0, DYNAMIC_TYPE_BIGINT);}
    if (ptrX) {XFREE(ptrX, 0, DYNAMIC_TYPE_BIGINT);}
    if (ptrP) {XFREE(ptrP, 0, DYNAMIC_TYPE_BIGINT);}
    return res;
}
#else   /* FREESCALE_LTC_TFM */
  fp_int   R[2];
...
static int _fp_to_unsigned_bin(int x, fp_int *t, unsigned char *b)
{
