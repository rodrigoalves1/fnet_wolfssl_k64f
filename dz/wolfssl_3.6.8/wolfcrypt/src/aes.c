...
/* AES-DIRECT */
        ...
        void wc_AesEncryptDirect(Aes* aes, byte* out, const byte* in)
        {
            ...
            MMCAU_AES_EncryptEcb(in, key, aes->rounds, out);
        }
        ...
        void wc_AesDecryptDirect(Aes* aes, byte* out, const byte* in)
        {
            ...
            MMCAU_AES_DecryptEcb(in, key, aes->rounds, out);
        }
...
/* AES-CBC */
    ...
    int wc_AesCbcEncrypt(Aes* aes, byte* out, const byte* in, word32 sz)
    {
        ...
        while (len > 0)
        {
            ...
            MMCAU_AES_EncryptEcb(temp_block, enc_key, aes->rounds, out + offset);
    ...
    int wc_AesCbcDecrypt(Aes* aes, byte* out, const byte* in, word32 sz)
    {
        ...
        while (len > 0)
        {
            ...
            MMCAU_AES_DecryptEcb(in + offset, dec_key, aes->rounds, out + offset);
...
int wc_AesGcmSetKey(Aes* aes, const byte* key, word32 len)
{
    ...
    #if defined (FREESCALE_LTC_AES_GCM)
    #elif defined(FREESCALE_MMCAU)
        MMCAU_AES_EncryptEcb(iv, rk, aes->rounds, aes->H);
    #else
                   ...
                   const byte* authIn, word32 authInSz)
{
    ...
    status_t retVal;
    retVal = LTC_AES_EncryptTagGcm(LTC_BASE,
                                         in,
                                         out,
                                         sz,
                                         iv,
    ...
    while (blocks--) {
        IncrementGcmCounter(ctr);
            ...
            #ifdef FREESCALE_MMCAU
                MMCAU_AES_EncryptEcb(ctr, key, aes->rounds, scratch);
            #else
                   ...
                   const byte* authIn, word32 authInSz)
{
    ...
    status_t retVal;
    retVal = LTC_AES_DecryptTagGcm(LTC_BASE,
                                         in,
                                         out,
                                         sz,
                                         iv,
     ...
     * cipher text. */
    {
        ...
        #ifdef FREESCALE_MMCAU
            MMCAU_AES_EncryptEcb(ctr, key, aes->rounds, EKY0);
        #else
...
static void roll_x(Aes* aes, const byte* in, word32 inSz, byte* out)
{
    ...
    while (inSz >= AES_BLOCK_SIZE) {
        ...
        #ifdef FREESCALE_MMCAU
            MMCAU_AES_EncryptEcb(out, key, aes->rounds, out);
        #else
                   ...
                   const byte* authIn, word32 authInSz)
{
    ...
    LTC_AES_EncryptTagCcm(LTC_BASE,
                                in,
                                out,
                                inSz,
                                nonce,
    ...
    #ifdef FREESCALE_MMCAU
        MMCAU_AES_EncryptEcb(B, key, aes->rounds, A);
    #else
                   ...
                   const byte* authIn, word32 authInSz)
{
    ...
    for (i = 0; i < lenSz; i++)
        B[AES_BLOCK_SIZE - 1 - i] = 0;
    #ifdef FREESCALE_MMCAU
        MMCAU_AES_EncryptEcb(B, key, aes->rounds, B);
    #else
