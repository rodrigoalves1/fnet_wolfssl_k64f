...
#include <wolfssl/wolfcrypt/hash.h>
    ...
    #define XGMTIME(c, t) gmtime((c))
    #define XVALIDATE_DATE(d, f, t) ValidateDate((d), (f), (t))
#elif defined(FREESCALE_KSDK_FREERTOS)
    #include <time.h>
    #include "FreeRTOS.h"
    #include "task.h"
    #define XTIME(t1)  local_time((t1))
    #define XGMTIME(c, t) gmtime((c))
    #define XVALIDATE_DATE(d, f, t) ValidateDate((d), (f), (t))
...
#define TIME_IN_SECONDS_1970_2016 1450656000

fnet_time_t ksdk_time(fnet_time_t *timer)
{
	fnet_time_t localTime;
	    if (timer == NULL)
	        timer = &localTime;

    *timer = fnet_timer_seconds() +TIME_IN_SECONDS_1970_2016;
    return *timer;
}

#endif /* FREESCALE_KSDK_BM */

#ifdef FREESCALE_KSDK_FREERTOS

/* this is to forward system time to current time - in 2016!! */
#define TIME_IN_SECONDS_1970_2016 1464129243
...
time_t local_time(time_t* timer)
{
    ...
    if (timer == NULL)
        timer = &localTime;
    *timer = (time_t)(((float)xTaskGetTickCount())/configTICK_RATE_HZ);
    return *timer + TIME_IN_SECONDS_1970_2016;
}

#endif /* FREESCALE_KSDK_FREERTOS */
