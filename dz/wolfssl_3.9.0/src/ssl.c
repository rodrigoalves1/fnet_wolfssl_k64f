...
#if defined(OPENSSL_EXTRA) || defined(HAVE_WEBSERVER)
    {
        ...
        if (line) {
            ...
            if (start && finish && (start < finish)) {
               ...
               // if ( XMEMCPY(info->name, start, finish - start)== FNET_NULL)
                XMEMCPY(info->name, start, finish - start);
                    //return SSL_FATAL_ERROR;
                info->name[finish - start] = 0;
               // if (XMEMCPY(info->iv, finish + 1, sizeof(info->iv)) == FNET_NULL)
                               ...
                               unsigned char* buf, int inLen, int* outLen)
{
    ...
    /* header */
    //if (XMEMCPY(buf, header, headerLen) == FNET_NULL)
    ...
    *outLen = inLen;  /* input to Base64_Encode */
    ...
    if ( (i + footerLen) > inLen)
        return BAD_FUNC_ARG;
    //if (XMEMCPY(buf + i, footer, footerLen) == FNET_NULL)
